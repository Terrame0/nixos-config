...
Never commit. The decision to commit is the user's; commit only when the user explicitly asks.

### Projects

All projects live under the home directory (`~/`). `~/nixos-config` is the NixOS + Home Manager configuration for this machine. It is a constant: system and user config always lives there.
...
Every git repository (you work in) should have an `.agent-docs/` directory at its root. It holds project-specific context that opencode reads when the task calls for it. Maintain it according to the writing docs conventions below.

Anything tied to one project, such as architecture, ongoing work, design decisions, tag models, or gotchas, goes in that project's `.agent-docs/`.
...
A change is not finished until the docs it contradicts are fixed in the same change. Whenever you rename, move, remove, or repurpose anything a doc describes, update the affected docs alongside the code.
